...
![1631772313481](/images/业务流程图.png)
...
![1633507880036](/images\5C1633507880036.png)

点击创建，稍作等待，项目就创建好了。

我们点击IDE右上角的运行项目按钮，就可以启动项目了。

![1633508222266](/images\5C1633508222266.png)

![1633508401229](/images\5C1633508401229.png)

可以看到，我们的基本项目已经跑起来了，在游览器输入127.0.0.1：5000，可以看见返回内容

![1633508441384](/images\5C1633508441384.png)
...
2. 找到当前项目的解释器，点击+号
   ...
   ![1633511229013](/images\5C1633511229013.png)

3. 点击管理库

   ![1633511263509](/images\5C1633511263509.png)

4. 修改镜像源 删掉默认的，新建 https://pypi.tuna.tsinghua.edu.cn/simple ，确定。

![1633511389633](/images\5C1633511389633.png)
...
![1633511990239](/images\5C1633511990239.png)
...
- 安装pymysql
  ...
  ![1633532606572](/images\5C1633532606572.png)

##### 3.5 修改项目结构

- 在项目目录下创建Financial的包，并创建项目的启动文件，main.py,删掉初始项目目录下的app.py.

![1633514529336](/images\5C1633514529336.png)

- 添加项目的配置文件

  项目目录下新建settings包，创建一个default.py文件，写入项目的配置信息

  ![1633515215611](/images\5C1633515215611.png)
  ...
  ![1633515344379](/images\5C1633515344379.png)
...
![1633514799001](/images\5C1633514799001.png)
...
![1633514893886](/images\5C1633514893886.png)
...
![1633516402738](/images\5C1633516402738.png)

![1633516453194](/images\5C1633516453194.png)
...
- 为项目安装redis包 flask-redis
  ...
  ![1633516929279](/images\5C1633516929279.png)

- 创建实例化redis的文件，并在app创建的时候初始化redis实例，绑定flask核心对象app

  comment包下面创建utils包，下面创建financial_redis.py文件，写入实例化redis方法

  ![1633517177370](/images\5C1633517177370.png)

  ![1633517253938](/images\5C1633517253938.png)
...
- models下 新建user.py 模型文件，对应数据库中t_user表。
  ...
  ![1633518912010](/images\5C1633518912010.png)
...
![1633526502735](/images\5C1633526502735.png)
...
- utils包下创建serializers.py
  ...
  ![1633578359181](images\5C1633578359181.png)
...
- 创建一个user相关的常量文件，保存相关设置参数
  ...
  ![1633528891185](/images\5C1633528891185.png)
...
- 创建user相关的api资源以实现restful风格的接口
  ...
  ![1633530414945](/images\5C1633530414945.png)
...
- 用户登录
  ...
  ![1634784945421](images\5C1634784945421.png)
  ...
  - 安装 包 pyjwt
    ...
    ![1634786207474](images\5C1634786207474.png)

  - 创建生成token和验证token的方法

    utils包下创建const.py （存放生成token相关的设置参数）

    ![1634786858230](images\5C1634786858230.png)
    ...
    ![1634786920965](images\5C1634786920965.png)
  ...
  - 登录的视图函数
    ...
    ![1634788321858](images\5C1634788321858.png)
  ...
  - 定义一个请求钩子函数，验证token，把验证成功之后的用户ID保存到全局变量g中
    ...
    ![1634793872534](images\5C1634793872534.png)
    ...
    ![1634793792319](images\5C1634793792319.png)

- 退出登录视图函数

  ![1634794889850](images\5C1634794889850.png)
...
- 创建一个自定义的json返回格式
  ...
  ![1633531322871](/images\5C1633531322871.png)
...
- 创建蓝图，将api资源类与蓝图绑定
  ...
  ![1634796030843](images\5C1634796030843.png)
...
- 在Flask实例app上注册蓝图
  ...
  ![1633540974844](/images\5C1633540974844.png)
...
![1633576565380](/images\5C1633576565380.png)
...
```
 ...
 ![1633576690242](/images\5C1633576690242.png)
...
- letter的分页序列化器是继承了基础的分页序列化基类，所以只要实现具体的序列化方法（get_object）即可,其他在基类里完成，调用的时候只需要在初始化时传入分页对象，并调用.to_dict()完成分页序列化
  ...
  ![1633581772644](images\5C1633581772644.png)

- letter包下新建letter_resource.py

  ![1633582874711](images\5C1633582874711.png)
...
![1633583502897](images\5C1633583502897.png)
...
- 在flask实例上注册蓝图
  ...
  ![1633583543774](images\5C1633583543774.png)
...
![1633584220003](images\5C1633584220003.png)
...
- 创建信件资源的序列化器，在card包下新建

- 分页对象Pagination的属性

  ![image-20240221211100354](C:\Users\123\AppData\Roaming\Typora\typora-user-images\image-20240221211100354.png)
...
- card包下新建card_resource.py
  ...
  ![1633586579309](images\5C1633586579309.png)
...
![1633586870567](images\5C1633586870567.png)
...
- 在flask实例上注册蓝图
  ...
  ![1633587073480](images\5C1633587073480.png)
...
![1633594558346](C:\Users\NINGMEI\AppData\Roaming\Typora\typora-user-images\1633594558346.png)
...
![1633678256444](images\5C1633678256444.png)
...
![1633596157515](images\5C1633596157515.png)
...
![1633596395628](images\5C1633596395628.png)
...
- 创建产品资源的序列化器，在product包下新建serializer.py
  ...
  ![1633677656308](images\5C1633677656308.png)
...
- product包下新建 product_resource.py
  ...
  ![1633678653701](images\5C1633678653701.png)
...
- 投资/交易记录的资源 
  ...
  ![1633679841972](images\5C1633679841972.png)
  ...
  - 交易记录序列化
    ...
    ![1634973647779](images\5C1634973647779.png)
  ...
  - 投资记录,交易记录资源
    ...
    ![1633680316305](images\5C1633680316305.png)
...
![1635301671995](images\5C1635301671995.png)
...
![1635301830413](images\5C1635301830413.png)
...
- 在flask实例上注册蓝图
  ...
  ![1634710386081](images\5C1634710386081.png)
...
- 创建交易流水号生成器和交易类型枚举类型
  ...
  - utils包下新建gen_trans_id.py
    ...
    ![1634540975538](images\5C1634540975538.png)

  - 交易类型

    resources包下transaction新建const.py

    ![1634701289406](images\5C1634701289406.png)

- resources下新建account包

- 创建账户资源的序列化器，在account包下新建serializer.py

  ![1633591462450](images\5C1633591462450.png)
...
- account包下新建account_resource.py
  ...
  ![1634709321218](images\5C1634709321218.png)
...
![1634710693664](C:\Users\NINGMEI\AppData\Roaming\Typora\typora-user-images\1634710693664.png)
...
- 在flask实例上注册蓝图
  ...
  ![1634711001694](images\5C1634711001694.png)
...
![1634712218513](images\5C1634712218513.png)
...
![1634714470821](images\5C1634714470821.png)
...
![1634800430315](images\5C1634800430315.png)
...
- 添加用户账号信息的序列化器
  ...
  ![1634799086712](images\5C1634799086712.png)
...
- 将资源绑定到API上
  ...
  ![1634801672659](images\5C1634801672659.png)
...
![1634804281526](images\5C1634804281526.png)
...
```
 ...
 ![1634804478262](images\5C1634804478262.png)
...
![1634869328058](images\5C1634869328058.png)
...
- 创建借款相关的资源
  ...
  - 债权资源 transaction包下 新建debt_resource.py
    ...
    ![1634806631275](images\5C1634806631275.png)
  ...
  - 债权还款  transaction包下 新建debt_repay_resource.py
    ...
    ![1634871494181](images\5C1634871494181.png)
  ...
  - 债权还款  transaction包下 新建loan_resource.py
    ...
    ![1634873724734](images\5C1634873724734.png)
...
![1635302085318](images\5C1635302085318.png)
...
![1634883054205](images\5C1634883054205.png)
...
- funding_not_matched.py(待匹配资金)
  ...
  ![1635131400601](images\5C1635131400601.png)
...
![1634884742754](images\5C1634884742754.png)
...
![1635302440678](images\5C1635302440678.png)
...
- 匹配结果
  ...
  ![1634895034519](images\5C1634895034519.png)
...
![1632541800241](images\5C1632541800241.png)

![1634961413046](images\5C1634961413046.png)
...
![1635302534475](images\5C1635302534475.png)
...
- 创建未匹配资金的序列化器
  ...
  ![1635132127233](images\5C1635132127233.png)
...
- 未匹配资金的API资源
  ...
  ![1635131908024](images\5C1635131908024.png)
...
- 创建续期收益的序列化器
  ...
  ![1635132680104](images\5C1635132680104.png)
...
- 预期收益的API资源
  ...
  ![1635133046739](images\5C1635133046739.png)

##### 14.3 提取收益

transaction包下新建investIncome_resource.py

![1635134550055](images\5C1635134550055.png)
...
![1635138540713](images\5C1635138540713.png)
...
- 匹配结果的API资源
  ...
  ![1635139612552](images\5C1635139612552.png)
...
- transaction包下init文件
  ...
  ![1635302990450](images\5C1635302990450.png)
